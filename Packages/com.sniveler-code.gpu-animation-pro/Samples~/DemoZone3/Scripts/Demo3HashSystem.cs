    ...
    public partial struct Demo3HashSystem : ISystem
    {
        ...
        public void OnCreate(ref SystemState state)
        {
            _aliveUnitsQuery = new EntityQueryBuilder(Allocator.Temp)
                .WithAll<LocalTransform, Demo3CombatData, AnimatorGpuIndex>()
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            const float mapSize = 100f;
            float inverseMicroCellSize = 1.0f / battle.MicroCellSize;

            _microGridWidth = (int)math.ceil(mapSize * inverseMicroCellSize);
            _microGridHeight = (int)math.ceil(mapSize * inverseMicroCellSize);
            ...
            {
                InverseMicroCellSize = inverseMicroCellSize,
                GridWidth = _microGridWidth,
                GridHeight = _microGridHeight,
                MapOffset = new float2(50f, 50f),
        ...
        {
            public float InverseMicroCellSize;
            ...
            private void Execute([EntityIndexInQuery] int index, in LocalTransform transform,
                in AnimatorGpuIndex gpuIndex, in Demo3CombatData combat)
            {
                float2 pos = transform.Position.xz + MapOffset;
                int cellX = math.clamp((int)(pos.x * InverseMicroCellSize), 0, GridWidth - 1);
                int cellY = math.clamp((int)(pos.y * InverseMicroCellSize), 0, GridHeight - 1);
                SpatialData[index] = new Demo3SpatialData
                {
                    CellIndex = (ushort)(cellY * GridWidth + cellX),
                    Position = transform.Position.xz,
                    Team = combat.Team,
                    GpuIndex = gpuIndex.Value
                };
            }
        }
